...
<script>
  ...
  export default {
    ...
    mounted(){
      if (this.$store.getters['products/wildberries/Products'].length === 0){
        this.$store.dispatch('products/wildberries/GetProducts')
      }
    },
    computed:{
      products(){
        return this.$store.getters['products/wildberries/Products'];
      },
    }
  }
</script>
