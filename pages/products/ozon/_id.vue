<template>
  <div class="lk-products">
    <div class="lk-products__head">
      <h1>Ozon</h1>
      <button type="button" class="lk-products__refresh" @click="UpdateProducts(parseInt($route.params.id) - 1)">Обновить данные</button>
    </div>
    <div class="lk-products__wrapper">
      <div class="lk-products__inner lk-products__content">
        <vue-custom-scrollbar class="lk-products__container" :tagname="'div'">
          <table v-if="(products[(parseInt(this.$route.params.id) - 1) + ''] !== undefined) && products[(parseInt(this.$route.params.id) - 1) + ''].length > 0">
            <thead>
            <tr role="row">
              <th>Название</th>
              <th>Артикул</th>
            ...
            <tbody>
            <tr role="row" v-for="(item, index) in products[(parseInt(this.$route.params.id) - 1) + '']" :key="`ozon-item-${index}`">
              <td>{{item.name}}</td>
              <td>{{item.offer_id}}</td>
              <td>{{item.id}}</td>
              <td>{{item.barcode}}</td>
      ...
      <div class="lk-products__inner lk-products__pagination">
        <div class="lk-products__pagination-wrapper">
          <nuxt-link v-for="(item, index) in Math.ceil(ProductsList.length / 10)" :to="`/products/ozon/${item}`" :key="`ozon-link-${index}`">{{item}}</nuxt-link>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    async mounted() {
      if ((Object.keys(this.products.length === 0) || (this.products[this.$route.params.id].length === 0))){
        await this.GetProducts(parseInt(this.$route.params.id) - 1);
      }

      if (this.$store.getters['products/ozon/ProductsList'].length === 0){
